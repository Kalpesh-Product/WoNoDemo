        ...
        tableData: {
          ...
          columns: [
           ...
           // { field: "projectedAmount", headerName: "Projected Amount (USD)", flex: 1 },
             { field: "projectedAmount", headerName: "Projected Amount(USD)",  flex: 1,valueFormatter: (params) => usdFormat(params.value), },
            {
              field: "actualAmount",
              headerName: "Actual Amount (USD)",
              flex: 1,
            },
  ...
  const financialData = Object.values(groupedData)
    .map((data, index) => {
      const transoformedRows = data.tableData.rows.map((row, index) => ({
        ...
        actualAmount: Number(
          row.actualAmount?.toLocaleString("en-US").replace(/,/g, ""),
      ...
      return {
        ...
        actualAmount: data.actualAmount.toLocaleString("en-US"), // Ensuring two decimal places for total amount
      ...
      bar: {
        ...
        dataLabels: {
          ...
          total: {
            ...
            //     ] || 0;
            //   return usdFormat(Number(total || 0));
            // },
            formatter: (_, config) => {
  const total =
    Number(
...
legend: {
  ...
  markers: {
    ...
    tooltip: {
      ...
      inverseOrder: true,
      y: {
        formatter: (value) => `USD ${usdFormat(Number(value || 0))}`,
      },
    },
  };
  ...
  return (
    <div className="flex flex-col gap-8">
      <FyBarGraph
        ...
        //titleAmount={`USD ${usdFormat(selectedFYActualAmount || 0)}`}
        selectedFY={selectedFiscalYear}
        onSelectedFYChange={setSelectedFiscalYear}
        includePointMeta
        tooltipValueMode="meta"
        disableHoverCrosshair
        TitleAmountGreen={`USD ${usdFormat(selectedFYActualAmount || 0)}`}
        TitleAmountTotal={`USD ${usdFormat(selectedFYProjectedAmount || 0)}`}
        greenTitle="Actual"
