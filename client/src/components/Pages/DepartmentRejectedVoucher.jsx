...
const DepartmentRejectedVoucher = () => {
  ...
  const { data: rejectedVouchers = [], isPending: isRejectedLoading } = useQuery({
   ...
   const columns = [
      ...
      // { field: "projectedAmount", headerName: "Particular Amount (USD)",flex: 1.5 },
     ...
     // { headerName: "Voucher Sr.No", field: "voucherSrNo", hide: true },
      ...
      { headerName: "Total Amount (USD)", field: "projectedAmount" },
    ...
    const tableData = rejectedVouchers.map((item, index) => ({
      ...
      projectedAmount: usdFormat(item.projectedAmount),
    ...
    ? item.particulars
        .map(
          (p, idx) =>
            `Particular ${idx + 1}: ${p.particularName || "-"} — USD ${usdFormat(
  ...
  //   { field: "projectedAmount", headerName: "Particular Amount (USD)" ,flex:1},
          ...
          <div className="flex flex-col gap-4">
           ...
           {Array.isArray(selectedBudget.particulars) &&
            selectedBudget.particulars.length > 0 ? (
              <>
                {selectedBudget.particulars.map((item, idx) => (
                  <DetalisFormatted
                    ...
                    detail={`${item.particularName || "-"} — USD ${
                      usdFormat(item.particularAmount || 0)
                    }`}
                  />
                ))}
              </>
              ...
              <DetalisFormatted
                title="Total Amount"
                // detail={`USD ${(selectedBudget.projectedAmount || 0).toLocaleString()}`}
                detail={`USD ${(selectedBudget.projectedAmount || 0).toLocaleString()}`}
              />
