...
const AssetsDashboard = () => {
  ...
  const getAssetGraphDate = (asset) =>
    // Attribute value to acquisition, not the date the record was imported.
  ...
  const assetUtilizationSeries = fiscalYears.map((fiscalYear) => {
    ...
    return {
      ...
      data: months.map((month) => {
        ...
        return total ? Number(Number(total).toFixed(2)) : 0;
      }),
    };
  });
    ...
    tooltip: {
      ...
      custom: ({ dataPointIndex }) => {
        ...
        const tooltipRows = [
          {
            key: "totalAssetValue",
            label: `Total Assets Value : <strong>USD ${usdFormat(total)}</strong>`,
            color: "#26C6DA",
          },
          {
            key: "assetCount",
  ...
  const assetsValueGraph = {
    ...
    titleAmount: `TOTAL ASSET VALUE : USD ${usdFormat(totalAssetsPrice)}`,
    ...
    },
    {
      ...
      widgets: [
        ...
        userPermissions.includes(PERMISSIONS.ASSETS_ASSET_VALUE.value) && (
          <DataCard
            title={"Total"}
            data={`USD ${usdFormat(totalAssetsPrice)}`}
