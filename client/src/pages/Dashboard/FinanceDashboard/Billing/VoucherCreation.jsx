...
const VoucherCreation = () => {
  ...
  const invoiceCreationColumns = [
    ...
    { field: "totalAmount", headerName: "Total Amount(USD)", flex: 1, valueFormatter: (params) => usdFormat(params.value), },
    { field: "advanceAmount", headerName: "Advance Amount(USD)", flex: 1, valueFormatter: (params) => usdFormat(params.value),hide: true },
             ...
             financeParticularSummary:
              Array.isArray(item.finance?.particulars) && item.finance.particulars.length > 0
                ? item.finance.particulars
                    .map((p) => `${p.particularName || "-"}: USD ${usdFormat(p.particularAmount || 0)}`)  
              .join(", ")
                : "-",
      ...
      {viewModal && viewDetails && (
        ...
        >
          <div className="space-y-3">
           ...
           <DetalisFormatted title="Total Amount" detail={`USD ${usdFormat(particularsTotal)}`} />
