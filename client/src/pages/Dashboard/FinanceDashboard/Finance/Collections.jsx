...
const Collections = () => {
  ...
  const barGraphOptions = {
    ...
    tooltip: {
      ...
      <div style="padding:10px;font-family:Poppins-Regular;font-size:13px;width:200px">
        ...
        <div style="display:flex; width : 100% ; justify-content : space-between"><strong>Amount</strong> USD ${usdFormat(
          paidAmount
        )}</div>
      </div>
    `;
  ...
  const handleExport = () => {
    ...
    const rows = exportRows.map((client, index) => [
      ...
      client.deskRate != null ? `USD ${usdFormat(client.deskRate)}` : "",
      client.revenue != null ? `USD ${usdFormat(client.revenue)}` : "",
  ...
  return (
    <div className="flex flex-col gap-8">
      <WidgetSection
        ...
        TitleAmount={`USD ${usdFormat(selectedGraph.paidTotal)}`}
        border
      >
        ...
        <WidgetSection layout={2}>
          <DataCard
            ...
            // description={`Current Month: ${sortedData[0]?.month || "N/A"}`}
             description={`Total : USD ${usdFormat(selectedGraph.paidTotal)}`}
            data={`${(completed / 12).toFixed(0) || 0}%`}
          />
          <DataCard
            title={"Due"}
            // description={`Current Month: ${sortedData[0]?.month || "N/A"}`}
            description={`Total : USD ${usdFormat(selectedGraph.dueTotal)}`}
      ...
      <WidgetSection
        ...
        TitleAmount={`USD ${usdFormat(currentMonthTotal)}`}
      >
      ...
      {viewDetails && (
        ...
        >
          <div className="space-y-3">
            ...
            <DetalisFormatted
              title="Desk Rate"
              detail={`USD ${usdFormat(viewDetails.deskRate)}`}
            />
            <DetalisFormatted
              title="Revenue"
              detail={`USD ${usdFormat(viewDetails.revenue)}`}
            />
            <br />
