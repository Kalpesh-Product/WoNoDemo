...
const FinanceDashboard = () => {
          ...
          <div style="padding: 10px; font-family: Poppins, sans-serif; font-size: 13px; width : 180px">
            ...
            <div style="margin-top: 6px;">
              ...
              <div style="display: flex; justify-content: space-between;">
                ...
                <span>USD ${usdFormat(expense) || "0"}</span>
              </div>
            </div>
          </div>
        `;
    ...
    descriptionData: [
      {
        ...
        value: `USD ${usdFormat(summaryMonthIncome)}`,
        route: "monthly-profit-loss",
      },
      {
        title: "Annual Average",
        value: `USD ${usdFormat(totalIncomeAmount / 12)}`,
        route: "annual-average-profit-loss",
      },
      {
        title: "Overall",
        value: `USD ${usdFormat(totalIncomeAmount)}`,
        route: "overall-profit-loss",
      },
      {
        title: "Per Sq. Ft.",
        value: `USD ${usdFormat(totalIncomeAmount / totalSqft)}`,
  ...
  const expenseCardData = {
    ...
    descriptionData: [
      {
        title: summaryMonthLabel,
        value: `USD ${usdFormat(summaryMonthExpense)}`,
        route: "monthly-profit-loss",
      },
      {
        title: "Annual Average",
        value: `USD ${usdFormat(totalExpense / 12)}`,
        route: "annual-average-profit-loss",
      },
      {
        title: "Overall",
        value: `USD ${usdFormat(totalExpense)}`,
        route: "overall-profit-loss",
      },
      {
        title: "Per Sq. Ft.",
        value: `USD ${usdFormat(totalExpense / totalSqft)}`,
        route: "sqft-wise-data",
      },
    ],
  };
  ...
  const netSavingsCardData = {
    ...
    descriptionData: [
      {
        title: summaryMonthLabel,
        value: `USD ${usdFormat(summaryMonthIncome - summaryMonthExpense)}`,
        route: "monthly-profit-loss",
      },
      {
        title: "Annual Average",
        value: `USD ${usdFormat((totalIncomeAmount - totalExpense) / 12)}`,
        route: "annual-average-profit-loss",
      },
      {
        title: "Overall",
        value: `USD ${usdFormat(totalIncomeAmount - totalExpense)}`,
        route: "overall-profit-loss",
      },
      {
        title: "Per Sq. Ft.",
      ...
      custom: function ({ seriesIndex }) {
        ...
        return `
          <div style="
            ...
            font-weight: 600;
          ">
            ${category?.label || ""}: USD ${category?.value?.toLocaleString("en-US") || 0}
          </div>
        `;
      },
    },
    ...
    tooltip: {
      y: {
        formatter: function (value) {
          return `USD ${usdFormat(value.toFixed(0))}`;
        },
      },
    },
  };
  ...
  const statutoryTooltipValues = statutoryDonutSeries.map(
    (amount, i) => `${statutoryDonutLabels[i]}: USD ${amount}`
  );
  //-----------------------------------------------------Donut Statutory Payments------------------------------------------------------//
...
const donutRentalSeries = [totalPaid, totalUnpaid];
const donutRentalTooltipValue = [
  ` USD ${totalPaid.toLocaleString("en-US")}`,
  ` USD ${totalUnpaid.toLocaleString("en-US")}`,
];
...
//-----------------------------------------------------Donut Rental Payments------------------------------------------------------//
  ...
  const marchPaymentColumns = [
    ...
    { id: "expanseType", label: "Type", width: 150 },
    {
      ...
      width: 150,
      renderCell: (row) => `${row.actualAmount.toLocaleString("en-US")}`,
    },
  ...
  const yearlyGraphConfigs = [
    {
      ...
      TitleAmountGreen: `USD ${usdFormat(totalIncomeAmount)}`,
      TitleAmountRed: `USD ${usdFormat(totalExpense)}`,
