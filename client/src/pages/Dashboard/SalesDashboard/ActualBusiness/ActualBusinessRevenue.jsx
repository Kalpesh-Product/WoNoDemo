...
const ActualBusinessRevenue = () => {
 ...
 const handleVerticalNavigation = (vertical) => {
  ...
  const tableData = selectedMonthData.map((domain, index) => ({
    ...
    revenue: ` ${usdFormat(domain.paidRevenue)}`,
  }));
      ...
      {!isTotalRevenue ? (
        <>
          <WidgetSection
            ...
            TitleAmount={`USD ${usdFormat(selectedMonthSummary.paid)}`}
            border
          >
          ...
          <WidgetSection
            ...
            TitleAmountTotal={`USD ${usdFormat(selectedMonthSummary.total)}`}
            TitleAmountGreen={`USD ${usdFormat(selectedMonthSummary.paid)}`}
            TitleAmountRed={`USD ${usdFormat(selectedMonthSummary.unpaid)}`}
            totalTitle="Total"
            greenTitle="Paid"
            redTitle="Unpaid"
