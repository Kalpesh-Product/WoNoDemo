...
const SalesDashboard = () => {
  ...
  const incomeExpenseOptions = {
    ...
    dataLabels: {
      enabled: true,
      formatter: (val) => (Number(val) > 0 ? usdFormat(val) : ""), // <-- format here
      style: {
        fontSize: "12px",
        colors: ["#000"],
      },
    ...
    tooltip: {
      enabled: false,
      y: {
        formatter: (val) => `USD ${usdFormat(val)}`,
      },
    },
  };
  //-----------------------------------------------------Graph------------------------------------------------------//
    ...
    descriptionData: [
      {
        ...
        value: `USD ${usdFormat(totalValue) || 0}`,
        route: "/app/dashboard/sales-dashboard/revenue/total-revenue",
      },
      {
        title: previousRevenueMonthLabel,
        value: `USD ${usdFormat(selectedSeries?.data?.[previousFiscalMonthIndex] || 0)}`,
        route: "/app/dashboard/sales-dashboard/revenue/total-revenue",
      },
      {
        title: "Closing Desks",
      ...
      {
        title: "Active Sq Ft",
        value: usdFormat(totalSqft) || 0,
        route: "/app/dashboard/sales-dashboard/mix-bag/inventory",
      },
      {
        title: "Per Sq. Ft.",
        value: usdFormat((totalValue || 0) / (totalSqft || 0)),
        route: "/app/dashboard/sales-dashboard/mix-bag/inventory",
      },
    ],
  };
 ...
 const totalFreeSeats = totalFreeInventoryFromUnits;
  ...
  const salesAverageData = {
    ...
    descriptionData: [
      {
        title: "Revenue",
        value: `USD ${usdFormat(totalValue / 12)}`,
        route: "/app/dashboard/sales-dashboard/revenue/total-revenue",
      },
      {
        title: "Occupied Desks",
