...
const [revenueBasePath] = location.pathname.split("/total-revenue");
  ...
  const financialDataForTable = months.map((monthLabel, i) => {
    ...
    //       revenue: usdFormat(category.data["2024-25"][i]),
    //       percentage: `${100}%`,
    //     }));
  ...
  const options = {
    ...
    //         <span>USD ${coworking.toLocaleString("en-US")}</span>
    //       </div>
    //     </div>
    ...
    //       <div style="display: flex; justify-content: space-between; width: 100%;">
    //         <span>Meetings</span>
    //         <span>USD ${meetings.toLocaleString("en-US")}</span>
    //       </div>
    //     </div>
    ...
    //         <span>USD ${virtualOffice.toLocaleString("en-US")}</span>
    //       </div>
    //     </div>
    ...
    //       <div style="display: flex; justify-content: space-between; width: 100%;">
    //         <span>Workation</span>
    //         <span>USD ${workation.toLocaleString("en-US")}</span>
    //       </div>
    //     </div>
    ...
    //         <span>USD ${altRevenue.toLocaleString("en-US")}</span>
    //       </div>
    //     </div>
    //   </div>
    // `;
  ...
  const tooltipBuilder = ({ monthLabel, rawDataMap, w, dataPointIndex }) => {
      ...
      .map(({ label, seriesName }) => {
        ...
        return `
          <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
            ...
            <div style="display: flex; justify-content: space-between; width: 100%;">
              <span>${label}</span>
              <span>USD ${value.toLocaleString("en-US")}</span>
            </div>
          </div>
        `;
      })
    ...
    return `
      <div style="padding: 10px; width: 300px">
        ...
        <div style="text-align: right; font-weight: 600;">Total: USD ${total.toLocaleString(
      "en-US",
        )}</div>
      </div>
    `;
    ...
    <div className="flex flex-col gap-4 ">
      ...
      <WidgetTable
        ...
        titleAmountTotal={({ dateRange }) => {
          ...
          return `USD ${usdFormat(summary.total)}`;
        }}
        titleAmountGreen={({ dateRange }) => {
          ...
          return `USD ${usdFormat(summary.paid)}`;
        }}
        titleAmountRed={({ dateRange }) => {
          ...
          return `USD ${usdFormat(summary.unpaid)}`;
        }}
        greenTitle="Paid"
        redTitle="Unpaid"
        totalTitle="Total"
