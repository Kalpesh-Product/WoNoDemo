    ...
    return [
      ...historicalPnlBaseIncomeData.map((income, index) => {
        ...
        return {
          ...
          totalIncome: usdFormat(income),
          totalExpense: usdFormat(expenseTotal),
          totalProfitLoss: usdFormat(income - expenseTotal),
        };
      }),
      ...historicalData.map((item, index) => ({
        // srNo: historicalPnlBaseIncomeData.length + index + 1,
        financialYear: item.fiscalYear,
        totalIncome: usdFormat(item.income),
        totalExpense: usdFormat(item.expense),
        totalProfitLoss: usdFormat(item.profitLoss),
      })),
    ];
  }
  ...
  return Object.values(groupedByUnits)
    ...
    .map((group, index) => {
      ...
      return {
        ...
        expense: expensePerSqFt.toLocaleString("en-US", {
