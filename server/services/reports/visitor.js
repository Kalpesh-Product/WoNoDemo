    ...
    switch (queryKey) {
      ...
      default:
        ...
        visitors = await Visitor.aggregate([
          ...
          // The dashboard reads department.name, just like populated list queries.
          // Resolve it even when no date filter is supplied, without dropping
          // visitors whose department is missing or has been removed.
          {
            $lookup: {
              from: "departments",
          ...
          {
            $set: {
              department: { $ifNull: [{ $first: "$department" }, null] },
            },
          },
